    ...
    if not isPullRequest:
        for step in [
            ...
            SetProperty(
                ...
                name="set-filename",
                property="filename",
                value=Interpolate("Expanse-%(prop:version)s-win64-%(kw:time_string)s-%(kw:short_revision)s.exe",
                                  time_string=get_time_string,
                                  short_revision=get_short_revision)
            ),
            FileUpload(
                name="upload",
                slavesrc=Interpolate("Expanse-%(prop:version)s-win64.exe"),
            ...
            MasterShellCommand(
                ...
                command=['rm', '-f', Interpolate("public_html/builds/%(prop:buildername)s/Expanse-win64-latest.exe")]
            ),
            MasterShellCommand(
                haltOnFailure=True,
                name="link-latest",
                ...
                command=['ln', '-sf', Interpolate("%(prop:filename)s"), Interpolate("public_html/builds/%(prop:buildername)s/Expanse-win64-latest.exe")]
            )
        ]: factory.addStep(step)
